...
class DeepMRM(nn.Module):
    """ Deep Masked Autoencoder with VisionTransformer backbone
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        self.decoder_pred1 = nn.Linear(decoder_embed_dim, (patch_size*2)**2 * in_chans, bias=True)  # decoder to patch
        self.decoder_pred2 = nn.Linear(decoder_embed_dim, (patch_size*2)**2 * in_chans, bias=True)  # decoder to patch
        self.decoder_pred3 = nn.Linear(decoder_embed_dim, (patch_size*2)**2 * in_chans, bias=True)  # decoder to patch
        self.decoder_pred4 = nn.Linear(decoder_embed_dim, (patch_size*2)**2 * in_chans, bias=True)  # decoder to patch
    ...
    def forward(self, batch, mask_ratio=0.75):
        ...
        loss1 = self.forward_loss(big_imgs, pred1, mask)
        loss2 = self.forward_loss(big_imgs, pred2, mask)
        loss3 = self.forward_loss(big_imgs, pred3, mask)
        loss4 = self.forward_loss(big_imgs, pred4, mask)

        report_loss = self.forward_report_decoder(latent12, ids, labels, attention_mask, type_ids)

        return (loss1, loss2, loss3, loss4, report_loss), pred4, mask
